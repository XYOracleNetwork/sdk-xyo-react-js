...
function run({
  ...
  const prodFiles = allFiles.filter(file => !file.endsWith('.spec.ts')
    && !file.endsWith('.stories.tsx') && !file.includes('/spec/') && !file.includes('/stories/'))
  const devFiles = allFiles.filter(file => file.endsWith('.spec.ts') || file.endsWith('.stories.tsx')
    || file.includes('/spec/') || file.includes('/stories/'))
  ...
  for (const imp of externalProdImports) {
    if (!deps.dependencies.includes(imp) && !deps.peerDependencies.includes(imp)) {
